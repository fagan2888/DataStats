...
def data_write(xian_zhong: str, tong: str, idate: IDate, conn: object):
    ...
    # 获取整体业务的相关数据
    zheng = Tong_Ji(name=name, risk="整体", conn=conn)
    # 获取不同险种的相关数据
    ...
    for month in yue_list:
        for ten_days in xun_list:
            # 通过旬数据的变化来判断行变换，从而实现隔行换背景色
            if ten_days % 2 == 1:
                wen_zi = sy.string
                shu_zi = sy.number
            ...
            ws.write_string(
                row=nrow,
                col=ncol,
                string=f"{month}月{xun_name[ten_days - 1]}",
                cell_format=wen_zi,
            )
            ncol += 1
            ...
            for year in nian_fen:
                ...
                ws.write(
                    ...
                    ncol,
                    fen.xun_bao_fei(
                        year=year, month=month, ten_days=ten_days, tong=tong
                    ),
                    shu_zi,
                )
                ncol += 1
                ...
                # 本年的同比数据只能是同期数据
                if year == idate.nian:
                    value = fen.xun_tong_bi(
                        year=year, month=month, ten_days=ten_days, tong=True
                    )
                else:
                    value = fen.xun_tong_bi(
                        year=year, month=month, ten_days=ten_days, tong=tong
                ...
                # 写入保费在整体业务中的占比
                if xian_zhong != "整体":
                    fen_value = fen.xun_bao_fei(
                        year=year, month=month, ten_days=ten_days, tong=tong
                    )
                    zheng_value = zheng.xun_bao_fei(
                        year=year, month=month, ten_days=ten_days, tong=tong
                    )
                ...
                #     value_sum = 0
                #     for i in range(1, 4):
                #         value_sum += fen.xun_bao_fei(
                #             year=year, month=month, ten_days=i, tong=tong
                #         )
                ...
                #         )
                # else:
                #     if fen.yue_bao_fei(year=year, month=month, tong=tong) == 0:
                #         value = "——"
                #     else:
                #         value = fen.xun_bao_fei(
                #             year=year, month=month, ten_days=ten_days, tong=tong
...
def ten_days_data(workbook: xlsxwriter.Workbook, company: str, conn: object) -> None:
    """
    写入旬数据的主函数
...
if __name__ == "__main__":
    ...
    a = datetime.now()
    ten_days_data(workbook=wb, company="分公司", conn=conn)
    b = datetime.now()
    wb.close()
    print(f"date {b-a=}")
