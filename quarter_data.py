...
def data_write(xian_zhong: str, tong: str, idate: IDate):
    ...
    # 设置统计季度
    ji_du_list: list = ["一季度", "二季度", "三季度", "四季度"]
    ...
    for ji in ji_du_list:
        # 通过季度数据的变化来判断行变换，从而实现隔行换背景色
        quarter = ji_du_list.index(ji)
        if quarter % 2 == 0:
            wen_zi = sy.string
            shu_zi = sy.number
        ...
        ncol = 0
        # 写入季度名称
        ws.write_string(row=nrow, col=ncol, string=ji, cell_format=wen_zi)
        ncol += 1
        ...
        for year in nian_fen:
            ...
            ws.write(
                nrow,
                ncol,
                fen.ji_bao_fei(year=year, quarter=quarter + 1, tong=tong),
                shu_zi,
            )
            ncol += 1
            ...
            # 本年的同比数据只能是同期数据
            if year == idate.nian:
                value = fen.ji_tong_bi(year=year, quarter=quarter + 1, tong=True)
            else:
                value = fen.ji_tong_bi(year=year, quarter=quarter + 1, tong=tong)
            ws.write(
            ...
            # 本年的环比数据只能是同期数据
            if year == idate.nian:
                value = fen.ji_huan_bi(year=year, quarter=quarter + 1, tong=True)
            else:
                value = fen.ji_huan_bi(year=year, quarter=quarter + 1, tong=tong)
            ws.write(
                nrow, ncol, value, bai_fen_bi,
            )
            ncol += 1
            ...
            # 写入保费在整体业务中的占比
            if xian_zhong != "整体":
                fen_value = fen.ji_bao_fei(year=year, quarter=quarter + 1, tong=tong)
                zheng_value = zheng.ji_bao_fei(
                    year=year, quarter=quarter + 1, tong=tong
                )
            ...
            # 写入保费在全年业务中的占比
            if tong is True:
                value = fen.ji_bao_fei(year=year, quarter=quarter + 1, tong=tong) / (
                    fen.ji_bao_fei(year=year, quarter=1, tong=tong)
                    + fen.ji_bao_fei(year=year, quarter=2, tong=tong)
                    + fen.ji_bao_fei(year=year, quarter=3, tong=tong)
                    + fen.ji_bao_fei(year=year, quarter=4, tong=tong)
                )
            else:
                value = fen.ji_bao_fei(
                    year=year, quarter=quarter + 1, tong=tong
...
def quarter_data(workbook: xlsxwriter.Workbook, company: str) -> None:
    """
    写入季度数据的主函数
...
if __name__ == "__main__":
    ...
    quarter_data(workbook=wb, company="分公司")
