...
def ji_gou_week(sh, sql):
    ...
    date_str = "数据统计范围：{0}至{1}".format(
        "2019-01-01", date.today()-timedelta(days=1))
    ...
    # 四级机构名称列表
    ji_gou_name = ("百大国际", "春怡雅苑", "香榭丽园", "宜良", "东川", "安宁", "春之城", "勐海", "勐腊",
                    "师宗", "陆良", "宣威", "罗平", "会泽", "沾益", "丘北", "砚山", "富宁", "马关",
                    "广南", "麻栗坡", "云龙", "宾川", "漾濞", "弥渡", "洱源", "祥云", "腾冲", "施甸",
                    "兰坪", "巧家")

    # 内部团队名称列表，在数据排序时机构与内部团队分开排序
    team_name = ("保山隆阳区营业部", "版纳中支本部", "怒江中支本部", "文山营业一部", "文山营业二部",
                 "曲靖中支本部", "曲靖营业一部", "大理中支本部", "分公司本部")
    ...
    bold_task_style = cell_style(
        height=12, bold=True, borders=True, num_format='0')
    num_style = cell_style(height=12, borders=True, num_format='0.00')
    bold_num_style = cell_style(
        height=12, bold=True, borders=True, num_format='0.00')
    percent_style = cell_style(height=12, borders=True, num_format='0.00%')
    bold_percent_style = cell_style(
        height=12, bold=True, borders=True, num_format='0.00%')
    percent_style_red = cell_style(
        height=12, font_color=0x0A, borders=True, num_format='0.00%')
    bold_percent_style_red = cell_style(
        height=12, font_color=0x0A, bold=True, borders=True, num_format='0.00%')
    ...
    for data in ji_gou_datas_sort:
        ...
        for key, value in enumerate(data):
            ...
            # 整体同比增长率列、时间进度达成率列、计划任务达成率列
            elif key in (18, 19, 20):
                if key == 20 and value >= 1:
                    sh.row(nrow + 2).write(ncol - 14,
                                           value, bold_percent_style_red)
                else:
                    sh.row(nrow + 2).write(ncol - 14,
                                           value, bold_percent_style)
