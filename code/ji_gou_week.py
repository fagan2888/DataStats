...
def ji_gou_week(sh, sql):
    ...
    for i in range(11, 14):
        sh.col(i).width = 256 * 12
    sh.write_merge(2, 2, 11, 13, "计划任务达成率", header_style)

    # 同比增长率列宽及第一行标题
    sh.col(14).width = 256 * 14
    sh.write_merge(2, 2, 14, 14, "同比增长率", header_style)
    ...
    for name in ji_gou_name:
        ...
        data = (name, che.task, fei_che.task, zheng_ti.task, che.this_year, fei_che.this_year, zheng_ti.this_year, che.time_progress_rate, 
                fei_che.time_progress_rate, zheng_ti.time_progress_rate, che.planned_task_completion_rate, 
                fei_che.planned_task_completion_rate, zheng_ti.planned_task_completion_rate, zheng_ti.year_tong_bi)
